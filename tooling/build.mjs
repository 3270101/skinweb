...
const client=await build({absWorkingDir:tooling,entryPoints:['src/client.js'],bundle:true,minify:true,format:'esm',target:['es2020'],jsx:'automatic',loader:{'.js':'jsx'},define:{'process.env.NODE_ENV':'"production"'},outdir:path.join(root,'assets'),entryNames:'skinow-[hash]',metafile:true,legalComments:'eof'});
const jsOutput=Object.keys(client.metafile.outputs).find(p=>p.endsWith('.js'));
const jsPath='/'+path.relative(root,path.resolve(tooling,jsOutput)).split(path.sep).join('/');
...
for(const name of await fs.readdir(path.join(root,'assets'))) {
  if(/^skinow-[a-zA-Z0-9]+\.(js|css)$/.test(name)&&!['/assets/'+name].some(value=>value===jsPath||value===cssPath))await fs.unlink(path.join(root,'assets',name));
}
