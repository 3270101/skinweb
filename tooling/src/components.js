...
export function ScalpCards() {
  return <div className="skin-grid skin-scalp-cards">{scalpServices.map(s=><article className="skin-card" key={s.slug} id={`scalp-${s.slug}`}>
    <p className="skin-eyebrow">{s.minutes} 分鐘</p><h3>{s.name}</h3><p>{s.description}</p>
    <p className="skin-note">原價 {money(s.original)}</p><p className="skin-price">會員價 {money(s.member)}</p>
  </article>)}</div>;
}
export function ScalpPriceTable() {
  return <div className="skin-table-wrap"><table className="skin-table"><caption>洗髮與頭皮養護價目表（新台幣）</caption>
    <thead><tr><th scope="col">服務</th><th scope="col">時間／類型</th><th scope="col">原價</th><th scope="col">會員價</th></tr></thead>
    <tbody>{[...scalpServices,scalpAddon].map(s=><tr key={s.slug} data-scalp={s.slug}><th scope="row"><a href="/services/scalp-care/">{s.name}</a></th><td>{s.minutes?`${s.minutes} 分鐘`:'加購項目'}</td><td>{money(s.original)}</td><td className="skin-price">{money(s.member)}</td></tr>)}</tbody>
  </table></div>;
}
export function ScalpVideo() {
  return <figure id="scalp-video" className="skin-video-block">
    ...
    <figcaption>完整影片約 2 分 34 秒，示範諮詢、頭皮檢查、洗髮沖洗、頭皮保養與吹整。實際服務內容與安排請於預約時向門市確認。</figcaption>
    <p className="skin-note"><a href={scalpVideo.src}>無法播放？開啟 MP4 影片</a></p>
  </figure>;
}
export function ScalpSection() {
  return <section id="scalp-care" className="py-16 bg-white"><div className="container mx-auto px-4">
    <h2 className="text-4xl font-bold text-center text-brand-primary mb-4">洗髮與頭皮深層養護</h2>
    <p className="text-center text-gray-600 mb-12">選擇最適合您的洗髮方案</p><ScalpCards/>
    <div className="skin-scalp-feature"><div className="skin-card"><p className="skin-eyebrow">加購項目</p><h3>{scalpAddon.name}</h3><p>{scalpAddon.description}</p><p className="skin-note">原價 {money(scalpAddon.original)}</p><p className="skin-price">會員價 {money(scalpAddon.member)}</p><p className="skin-note">提供門市、適用方案與時段請於預約時確認。</p><BookingLink>詢問頭皮養護與預約</BookingLink><p><a className="skin-text-link" href="/services/scalp-care/">查看完整服務介紹與價格 →</a></p></div><div><h3 className="skin-video-title">洗髮流程與服務介紹影片</h3><ScalpVideo/></div></div>
  </div></section>;
}
...
export function ExploreSection() {
  return <section id="explore" className="py-16 bg-gray-50"><div className="container mx-auto px-4">
    ...
    <nav className="skin-link-row" aria-label="護膚資訊導覽">
      <a href="/services/">所有服務</a><a href="/services/scalp-care/">頭皮養護與影片</a><a href="/services/manual-extraction/">手工清粉刺</a><a href="/pricing/">價目表</a><a href="/membership/">會員制度</a><a href="/stores/">門市資訊</a><a href="/faq/">常見問題</a>
    </nav>
  </div></section>;
}
...
export function PageBody({route}) {
  if(route.kind==='services') return <><p>比較護膚、洗髮與頭皮養護方案的服務重點、所需時間與服務內容；所有費用均以新台幣計價。</p><h2>臉部護膚方案</h2><ServiceLinks/><AddonCards/><h2>洗髮與頭皮深層養護</h2><ScalpCards/><h3>{scalpAddon.name}加購</h3><p>{scalpAddon.description}</p><p>原價 {money(scalpAddon.original)}，會員加購價 {money(scalpAddon.member)}；適用方案請向門市確認。</p><p><a href="/services/scalp-care/#scalp-video">觀看洗髮流程影片與完整介紹 →</a></p><p><a href="/pricing/">比較完整價目表</a></p></>;
  if(route.kind==='scalp') return <><p className="skin-lead">選擇最適合您的洗髮方案，從深層洗髮、頭皮調理到頭皮深層養護，了解不同服務的時間、價格與護理重點。</p><h2>選擇洗髮與頭皮養護方案</h2><ScalpCards/><h2>水珍柔光精粹加購</h2><p>{scalpAddon.description}</p><p>原價 {money(scalpAddon.original)}，會員加購價 {money(scalpAddon.member)}。適用的洗髮方案與安排請向門市確認。</p><h2>洗髮流程與服務介紹影片</h2><ScalpVideo/><h2>洗髮與頭皮養護價目表</h2><ScalpPriceTable/><p>專屬會員卡優惠價 {money(membership.price)}（原價 {money(membership.original)}），效期 {membership.months} 個月；<a href="/membership/">查看會員制度</a>。</p><h2>如何預約？</h2><p>請透過官方 LINE 告知希望預約的方案、日期與時段，確認提供服務的門市、加購項目及當次安排。</p><BookingLink>詢問洗髮與頭皮養護</BookingLink></>;
